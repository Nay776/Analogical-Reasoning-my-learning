...
'''learning prompt的'''
class SPARModel(DetectionModel):
    def __init__(self, cfg="yolov8s.yaml", ch=3, nc=10, verbose=True):
        """Initialize YOLOv8 world model with given config and parameters."""
        self.txt_feats = torch.randn(1, nc or 80, 512)  # features placeholder
        self.clip_model = None  # CLIP model placeholder
        self.learnable_ctx = torch.randn(nc or 80,7, 512) * 0.02
        super().__init__(cfg=cfg, ch=ch, nc=nc, verbose=verbose)
        self.learnable_ctx = nn.Parameter(torch.randn(nc or 80,7, 512) * 0.02)
        self._register_ema_compatible_params()  # 重新注册参数
    ...
    def set_classes(self, text, batch=80, cache_clip_model=True):
        ...
        device = next(model.parameters()).device
        # text_token = clip.tokenize(text).to(device)
        text_token = torch.cat([clip.tokenize(p) for p in text]).to(device)
        tokenized_prompts = model.token_embedding(text_token)[:,:70,:].type(model.dtype)
        learnable_ctx=self.learnable_ctx.to(device).to(dtype).detach().clone()
        ...
        txt_feats = x[torch.arange(x.shape[0]), text_token.argmax(dim=-1)] @ model.text_projection

        # txt_feats = [model.encode_text(token).detach() for token in text_token.split(batch)]
        txt_feats = txt_feats / txt_feats.norm(p=2, dim=-1, keepdim=True)
        self.txt_feats = txt_feats.reshape(-1, len(text), txt_feats.shape[-1]).detach().clone()
        self.model[-1].nc = len(text)

    def predict(self, x, profile=False, visualize=False, txt_feats=None, augment=False, embed=None):
        # self.set_classes(['pedestrian', 'people', 'bicycle', 'car', 'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor'])
        txt_feats = (self.txt_feats if txt_feats is None else txt_feats).to(device=x.device, dtype=x.dtype)
        if len(txt_feats) != len(x) or self.model[-1].export:
            txt_feats = txt_feats.expand(x.shape[0], -1, -1)
        ori_txt_feats = txt_feats.clone()
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is AnaC2f:  # set 1) embed channels and 2) num heads
            ...
            {Detect, CLIPDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect}
        ):
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, YOLOESegment)):
                ...
                return "obb"
            elif isinstance(m, (Detect, CLIPDetect, YOLOEDetect, v10Detect)):
                return "detect"
